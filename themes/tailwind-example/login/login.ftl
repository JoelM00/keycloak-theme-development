				...
				<div>
					...
					<div class="providers">
						<#list social.providers as p>
							<div class='provider'>
								<a type="button" href="${p.loginUrl}" id="social-${p.alias}" class="provider-link bg-white ">
				...
				<form class="space-y-6" id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
					<div>
						...
						<div class="mt-1 relative">
							<input tabindex="1" id="username" name="username" value="${(username!'')}" type="text" autofocus autocomplete="off" required class="input" aria-invalid="<#if messagesPerField.existsError('username','password')>true</#if>">
					...
					<div class="mt-1">
						<input tabindex="2" id="password" name="password" type="password" aria-invalid="<#if messagesPerField.existsError('username','password')>true</#if>" autocomplete="current-password" class="input">
					</div>
					</div>

					<div>
						<input type="hidden" id="id-hidden-input" name="credentialId" <#if auth.selectedCredential?has_content>value="${auth.selectedCredential}"</#if>>
						<button tabindex="4" type="submit" name="login" id="kc-login" class="signin">
							Iniciar Sessão
						</button>
					</div>
				</form>
